...
| **Unit tests** (Vitest, 52 tests) | Cover csv escaping, filter/sort/summarize, validation, formatting, categorization, the health score, and flag toggling. | The pure logic is where bugs hide; tests make the behavior explainable and regression-safe. |
| **Dark theme** | SOC-style dark palette driven by CSS variables. | Dashboards run on wall displays; dark makes the severity colors pop and reduces eye strain. Variable-driven so the whole site themes from one place. (A green "Matrix" variant was tried and reverted — the dark palette read better.) |
